...
function parseKeyFromUrl(url2) {
  const matches = url2.match(/figma\.com\/(board|design|file)\/([^/?]+)/);
  if (matches) {
    return matches[2];
  }
  throw new Error("Could not parse Figma key from URL");
}
async function downloadFigmaFile(key) {
  const response = await axios_default.get(`https://api.figma.com/v1/files/${key}`, {
    headers: {
...
async function getThumbnails(key, ids) {
...
async function readComments(fileKey) {
  const response = await axios_default.get(`https://api.figma.com/v1/files/${fileKey}/comments`, {
    headers: {
      "X-FIGMA-TOKEN": getFigmaApiKey()
    }
  });
...
async function postComment(fileKey, message, x, y, nodeId) {
...
async function imageUrlToBase64(url2) {
...
async function doAddFigmaFile(url2) {
  const key = parseKeyFromUrl(url2);
...
async function doViewNode(fileKey, nodeId) {
  ...
  const b64 = await imageUrlToBase642(nodeThumb);
  return {
    content: [
      {
        type: "text",
...
async function doReadComments(fileKey) {
...
async function doPostComment(fileKey, message, x, y, nodeId) {
...
async function doReplyToComment(fileKey, commentId, message) {
...
async function imageUrlToBase642(url2) {
